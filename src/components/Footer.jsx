            ...
            <div key={footerLink.title} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              ...
              <ul className="list-none mt-4">
                {footerLink.links.map((link,index)=>(
                  <li key={link.name}
                  className={`font-poppins font-normal text-[16px] leading-[24px] text-slate-500 hover:text-cyan-500 cursor-pointer ${index!==
      ...
      border-t-[#3F3E46]">
        ...
        <div className="flex flex-row">
          {socialMedia.map((social,index)=>(
            ...
            ${index!==social.length-1 ? "mr-6" : "mr-0"}`} onClick={()=>(window.open(social.link))}/>
          ))}
        </div>
      </div>
    </section>
